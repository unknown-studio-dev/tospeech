...
done < <(/usr/bin/find "$native_root/EchoLab/Features" "$native_root/EchoLab/App/AppRootView.swift" -name '*.swift' -type f -print0)

shadowing="$native_root/EchoLab/Features/Shadowing"
require_source() {
  local file="$1"
  local pattern="$2"
...
for component in \
  'ShadowingPracticeScaffold(' 'ShadowingReviewScaffold(' \
  'VideoPreviewView(' 'TranscriptNavigatorView(' \
  'SentenceView(' 'PracticeTransportView(' 'ReviewPanelView(' 'RepeatOptionsView('
do
