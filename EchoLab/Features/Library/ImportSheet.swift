    ...
    EchoDialog(
      ...
      VStack(alignment: .leading, spacing: 14) {
        VStack(alignment: .leading, spacing: 8) {
          ...
          Text("Chỉ tải audio. Video phát online khi bạn bật; không lưu video về máy.")
            .font(EchoFont.body(size: 13)).foregroundStyle(EchoTheme.muted)
        }
        VStack(alignment: .leading, spacing: 8) {
          Text("Or choose local audio").font(EchoFont.body(size: 13, weight: .semibold))
          HStack {
            Group {
        ...
        DisclosureGroup("Advanced preparation options") {
          VStack(alignment: .leading, spacing: 10) {
            ...
            EchoCheckbox(title: "Chuẩn bị bản dịch tiếng Việt", isOn: $translate)
      ...
      HStack {
        Text("Nothing is uploaded in this preview.").font(EchoFont.body(size: 12)).foregroundStyle(
          EchoTheme.muted)
        Spacer()
