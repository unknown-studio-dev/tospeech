  ...
  var body: some View {
    VStack(spacing: 8) {
      ZStack {
        ...
        .contextMenu {
          Button("Chỉnh timing", action: onEdit) // native-control: menu
          if !isProductionMode {
      ...
      HStack(spacing: 8) {
        Label {
          EchoLocalizedText(
            store.preferences.video ? effectiveState.rawValue : "Video tắt · đang dùng audio gốc")
        } icon: {
          Image(systemName: "speaker.slash")
        }
        .font(EchoFont.metadata).foregroundStyle(EchoTheme.secondaryText).lineLimit(1)
        Spacer(minLength: 0)
        if store.preferences.video
