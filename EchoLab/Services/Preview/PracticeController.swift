...
final class PracticeController {
  ...
  func playSentence(repeating: Bool = false) {
    guard interrupt(), let sentence = store.selectedSentence else { return }
  ...
  func sourceFinished() {
    ...
    hasListened = true
    if captureAfterSource || (repeatEnabled && roundPreferences.autoRecord) {
  ...
  func playingWordID(in sentence: LessonSentence) -> String? {
    ...
    let matches = sentence.words.filter { word in
      ...
      return interval.start <= sourcePosition && sourcePosition < interval.end
    }
