    ...
    EchoDialog(
      ...
      HStack {
        ...
        EchoButton("Chuẩn bị bài", symbol: "arrow.down.circle", kind: .primary) {
          ...
          Task {
            if let job = await model.submit(request, localeIdentifier: store.preferences.accent == .uk ? "en-GB" : "en-US", transcriptionEngine: store.preferences.transcriptionEngine, transcriptionModelID: TranscriptionSelection.parakeet.modelID, compareWithApple: store.preferences.compareTranscriptWithApple) {
              onSubmitted(job)
            }
          }
        }
