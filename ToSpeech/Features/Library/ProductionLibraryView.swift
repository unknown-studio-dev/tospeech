  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 24) {
        ...
        if !model.hasLoaded {
          EchoLoading(title: "Loading").frame(maxWidth: .infinity, minHeight: 180)
      ...
      .echoContentReveal(value: model.hasLoaded)
    }
    .sheet(
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 9) {
      ZStack(alignment: .topTrailing) {
        ...
        EchoIconButton(
          symbol: "trash", label: EchoLocalization.format("library.delete_lesson", locale: locale, arguments: [lesson.title]),
          dark: true, action: delete
        ).padding(8)
      }
      Text(lesson.title).font(EchoFont.heading(size: 17, weight: .medium)).lineLimit(2)
      EchoLocalizedText(lesson.duration.map(EchoFormat.time) ?? "Audio unavailable")
        .font(EchoFont.body(size: 12)).foregroundStyle(EchoTheme.muted)
      if lesson.lifecycle == .ready {
        EchoLocalizedText(
          lesson.isPracticeReady
            ? EchoCopy("library.prepared_sentences", arguments: [.raw("\(lesson.preparedSentenceCount)")])
            : EchoCopy("Lesson preparation needs a transcript and sentence timing.")
        )
