...
struct RecordingModelsSettingsView: View {
  ...
  @Environment(\.pronunciationModelManager) private var pronunciationModels
  ...
  var body: some View {
    SettingsColumns(composition: .recording) {
      ...
      VStack(alignment: .leading, spacing: 32) {
        ...
        if let pronunciationModels { ProductionAssessmentModelsView(manager: pronunciationModels) }
        else { models }
      }
    }
  ...
  private var recording: some View {
    ...
    return SettingsSection(title: "Ghi âm", subtitle: "Nghe xong → đếm ngược → ghi âm.", spacing: 20) {
      Toggle(isOn: $store.preferences.enhanceRecordings) {
        EchoLocalizedText("recording.enhance.title").font(EchoFont.body(size: 16))
      }.toggleStyle(EchoToggleStyle(minimumHeight: 40, fillsWidth: true, labelFirst: true))
      EchoLocalizedText("recording.enhance.help")
