...
struct RepeatOptionsView: View {
  ...
  var body: some View {
    EchoDialog(
      ...
      VStack(alignment: .leading, spacing: 10) {
        ...
        Toggle(isOn: $draft.autoRecord) {
          VStack(alignment: .leading, spacing: 4) {
            EchoLocalizedText("Record after every listen").font(EchoFont.body(size: 13, weight: .medium))
            EchoLocalizedText("Listen finishes before countdown and capture.").font(EchoFont.metadata)
              .foregroundStyle(EchoTheme.muted)
          }
        }.toggleStyle(EchoToggleStyle(fillsWidth: true, contentPadding: 10)).background(
          EchoTheme.selected, in: RoundedRectangle(cornerRadius: 10))
        EchoDisclosureGroup("Advanced options", isExpanded: $advanced) {
      ...
      HStack {
        EchoLocalizedText("Independent take per round.").font(EchoFont.metadata).foregroundStyle(EchoTheme.muted)
        Spacer()
