...
struct InlineFeedbackRuntimeActions {
  ...
  var onRetry: () -> Void
  var onReview: () -> Void
  var matchingState: InlineFeedbackState? = nil
  var matchingMessage: String? = nil
...
struct InlineTakeFeedbackRow: View {
  ...
  private var state: InlineFeedbackState { runtime?.matchingState ?? InlineFeedbackState(take: take) }
    ...
    HStack(spacing: 16) {
      ...
      VStack(alignment: .leading, spacing: 6) {
        Text(verbatim: runtime == nil
          ...
          : EchoLocalization.format("matching.take_metadata", locale: locale, arguments: [take.number]))
        ...
        if let word = runtime?.errorWord, state == .failed {
          Text(verbatim: EchoLocalization.format("assessment.error.word", locale: locale, arguments: [word]))
  ...
  @ViewBuilder private var actions: some View {
    HStack(spacing: 8) {
      ...
      case .failed:
        ...
        if let runtime {
          EchoButton("Xem bản thu", symbol: "arrow.up.right", size: .regular, action: runtime.onReview)
        }
  ...
  private var message: String {
    if let matchingMessage = runtime?.matchingMessage { return matchingMessage }
    return switch state {
