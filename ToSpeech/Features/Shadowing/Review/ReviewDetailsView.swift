...
struct ReviewSoundDetailView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      if !hasEvidence {
        EchoLocalizedText("Recorded sound detail is unavailable for this take.").font(EchoFont.body(size: 11))
...
struct ReviewDeliveryDetailView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      HStack {
        EchoLocalizedText("Pitch, stress & rhythm comparison").font(EchoFont.body(size: 13, weight: .semibold))
        Spacer()
  ...
  private func dimension(_ title: String) -> some View {
    VStack(alignment: .leading, spacing: 3) {
      ...
      EchoLocalizedText("Unavailable").font(EchoFont.body(size: 11, weight: .semibold))
    }
