...
struct ReviewRuntimePresentation {
  ...
  var matchingService: ContentMatchingService? = nil
  var onMatch: (() -> Void)? = nil
  var assessmentService: PronunciationAssessmentService? = nil
  var onAssess: (() -> Void)? = nil
  var onReplayDetail: ((Double, Double) -> Void)? = nil
  var onReferenceDetail: ((Double, Double) -> Void)? = nil
  var onCompareDetail: ((AudioSpan, AudioSpan) -> Void)? = nil
  var onStop: (() -> Void)? = nil
  var player: ProductionAudioPlayer? = nil
  var sourceAudioURL: URL? = nil
  var onCompareTogether: (() -> Void)? = nil
  var sourceAsset: ReviewAudioAsset? = nil
  var takeAssets: [ReviewAudioAsset] = []
  var onManageRecordings: (() -> Void)? = nil
}
...
struct ReviewPanelView: View {
      ...
      VStack(alignment: .leading, spacing: 0) {
        VStack(alignment: .leading, spacing: 6) {
          ...
          Text(verbatim: EchoLocalization.format(
            ...
            arguments: [take.number, runtime?.assessmentService?.history(takeID: UUID(uuidString: take.id) ?? UUID()).last.map { _ in "Buddy · English v1" } ?? selectedAssessment?.engine.title
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 14) {
            ...
            if let assessment = completeAssessment {
              ...
              !(runtime?.matchingService != nil && take.outcome == .complete) {
              EchoNotice(
                text: statusTitle, error: take.outcome == .noSpeech || take.outcome == .quiet)
            }
            if let service = runtime?.assessmentService, let takeID = UUID(uuidString: take.id),
              [.complete, .earlyStop].contains(take.outcome) {
              PronunciationReviewView(history: service.history(takeID: takeID), error: service.error,
                onRetry: { job in Task { await service.retry(job) } }, onRecover: { Task { await service.recover() } },
                onAssess: { runtime?.onAssess?() }, onReplay: { runtime?.onReplayDetail?($0, $1) },
                onSourceReplay: { runtime?.onReferenceDetail?($0, $1) })
            }
            if let service = runtime?.matchingService, let takeID = UUID(uuidString: take.id),
              [.complete, .earlyStop].contains(take.outcome) {
              ContentMatchingReviewView(history: service.history(takeID: takeID), error: service.error,
                onRetry: { job in Task { await service.retry(job) } },
                onRecover: { Task { await service.recover() } })
            }
            playback
            if completeAssessment != nil { priorities }
            if completeAssessment != nil {
              EchoDisclosureGroup("Sounds & word detail", isExpanded: $soundOpen) {
              ...
              EchoDisclosureGroup("Pitch, stress & rhythm", isExpanded: $deliveryOpen) {
                ReviewDeliveryDetailView(assessment: selectedAssessment).padding(.top, 8)
              }.tint(EchoTheme.muted).font(EchoFont.body(size: 13))
            }
            if !take.assessments.isEmpty {
              EchoDisclosureGroup("Engine details") { assessmentHistory.padding(.top, 8) }
        ...
        VStack(alignment: .leading, spacing: 12) {
          HStack(spacing: 8) {
            if let action = runtime?.onMatch {
              EchoButton("matching.rerun", kind: .ghost, action: action)
                .disabled(![.complete, .earlyStop].contains(take.outcome)
                  || runtime?.matchingService?.history(takeID: UUID(uuidString: take.id) ?? UUID()).contains(where: { $0.isPending }) == true)
            } else if let unavailable = runtime?.assessmentUnavailableText {
              Text(unavailable).font(EchoFont.body(size: 10)).foregroundStyle(EchoTheme.muted)
            } else {
              EchoLocalizedText("Re-scoring appends a result and keeps this history.")
  ...
  private var reviewTitle: String {
    if runtime?.assessmentService != nil, [.complete, .earlyStop].contains(take.outcome) { return "assessment.title" }
    if runtime?.matchingService != nil, [.complete, .earlyStop].contains(take.outcome) { return "matching.title" }
