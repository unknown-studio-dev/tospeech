...
@MainActor final class AppLifecycle: NSObject, NSApplicationDelegate {
  ...
  func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
    ...
    guard store?.practice.interrupt() != false else { return .terminateCancel }
    // A dictation draft reaches the database 350 ms after the last keystroke and
    // its clock is stored on pause. Quitting inside that window must not lose
    // either, so the pending writes finish before the app goes away.
    if let dictation = productionShadowing?.dictation, dictation.lessonID != nil {
      dictation.suspend()
      Task { @MainActor in
