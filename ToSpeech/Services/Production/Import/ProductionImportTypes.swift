...
enum ProductionImportError: Error, Equatable, LocalizedError, Sendable {
  ...
  var presentationDescription: String {
    ...
    if technical.contains("transcription engine returned no word transcript") {
      return "transcription.import.empty"
    }
    ...
    case .unsupportedMedia:
      return "This source does not contain audio that ToSpeech can prepare."
    case .toolchain:
      return "The local import tools are unavailable. Reopen ToSpeech, then try again."
    case .subprocess, .recoveryRequired:
      return "ToSpeech could not prepare this lesson. Try again or import an audio file."
    case .persistence:
      return "ToSpeech could not save the import locally. Check free storage, then try again."
    case .modelNotInstalled:
      return "Download a transcription model in Settings, then import again."
    }
  }
