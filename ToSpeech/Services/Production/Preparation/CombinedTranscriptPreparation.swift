...
struct CombinedTranscriptArchive: Codable, Sendable {
  ...
  /// Legacy field from the removed Whisper engine (2026-09-14); kept so stored records still decode.
...
struct CombinedWordDecision: Codable, Equatable, Sendable {
  /// Legacy Whisper field; always nil for new records.
  ...
  static func prepare(
    ...
    for cue in NaturalSentenceSegmenter.segment(primary.words) {
      ...
      for (index, word) in base.enumerated() {
        ...
        let validPrimary = word.start.isFinite && word.end.isFinite && word.end > word.start && word.start >= 0
        var text = word.text
        ...
        decisions.append(CombinedWordDecision(
          whisperText: nil, primaryText: word.text, appleText: match?.candidateText, selectedText: text,
      ...
      let evidence = CombinedTranscriptEvidence(
        policy: "youtube-primary-apple-v2", whisperModel: nil,
