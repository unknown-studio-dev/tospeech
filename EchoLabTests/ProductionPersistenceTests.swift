...
struct ProductionPersistenceTests {
  @Test func backendPathsCreateOnlyTheDeclaredProductionLayout() throws {
    ...
    for directory in [
      ...
      paths.finalTakes, paths.packages, paths.cache,
    ] {
  ...
  @Test func deletionFenceAdvancesGenerationAndRejectsStaleCallbacks() async throws {
    let root = temporaryRoot()
    defer { try? FileManager.default.removeItem(at: root) }
    let database = try ProductionDatabase(url: root.appendingPathComponent("echolab.sqlite3"))
  ...
  @MainActor @Test func retainedTakeManifestCommitsAfterRelaunch() async throws {
    let root = temporaryRoot()
    defer { try? FileManager.default.removeItem(at: root) }
    let fixture = try await preparedPracticeFixture(root: root)
    ...
    try writeAudioFixture(to: staging)
  ...
  private func preparedPracticeFixture(root: URL) async throws -> (
    ...
    let sql = """
      ...
      SELECT '\(revisionID.uuidString)', '\(segmentID.uuidString)', 1, 'Practice target', 'content', 'reference', id, 0, frame_count, 1, '[]', '{}', 1800000000 FROM media_assets WHERE id = '\(audioAssetID.uuidString)';
  ...
  private func temporaryRoot() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent(
      "EchoLab-production-test-\(UUID().uuidString)", isDirectory: true)
  }
}
