...
struct ProductionWordTimingTests {
  ...
  @Test func reviewDoesNotEraseObservedWordPlaybackOrHighlight() {
    ...
    #expect(ProductionWordTiming.playingWordID(at: 1800, tokens: [word], in: target) == nil)
  }
  ...
  @Test func overlapsSilenceAndPunctuationDoNotInventHighlight() {
    ...
    #expect(ProductionWordTiming.playingWordID(at: 1900, tokens: words, in: target) == nil)
    #expect(ProductionWordTiming.playingWordID(at: 2100, tokens: words, in: target) == "world")
    #expect(ProductionWordTiming.playingWordID(at: 2550, tokens: words, in: target) == nil)
    #expect(ProductionWordTiming.playingWordID(at: 3000, tokens: words, in: target) == nil)
  }
}
