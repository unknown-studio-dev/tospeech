...
struct CombinedTranscriptTests {
  ...
  private func prepare(_ whisper: [TimedWord], _ apple: [TimedWord], captions: [CaptionCue] = [], source: TranscriptSource? = nil) throws -> [PreparedLessonSegment] {
    try CombinedTranscriptPreparation.prepare(whisper: whisper, variant: .large,
  ...
  @Test func bothASRsAgreeAndProvenanceSurvivesTimingEdit() throws {
    ...
    #expect(first.reconciliation?.whisperModel == "large-v3")
  ...
  @Test func appleFillsOnlyMatchingMissingWordTiming() throws {
    let whisper = [TimedWord(text: "Hello", start: 0, end: 0), TimedWord(text: "world.", start: 1, end: 1.5)]
    let result = try prepare(whisper, words("Hello world."))
    let value = try baseline(result[0])
    #expect(value.originalTokens?[0].endFrame == 500)
    #expect(value.reconciliation?.words[0].timingSource == .appleSpeechAnalyzer)
    let different = try prepare(whisper, words("Goodbye world."))
  ...
  @Test func hallucinatedSentenceAfterEOFDoesNotDiscardValidTranscript() throws {
    ...
    let value = try baseline(result[0])
    #expect(value.reconciliation?.excludedWhisperWords?.map(\.text) == ["Thank", "you."])
    #expect(value.wordTimingNeedsReview)
  }
  ...
  @Test func stageCacheIsBoundToAudioModelLocaleAndRuntime() throws {
    ...
    let key = ImportTranscriptCacheIdentity(sourceChecksum: "audio1", engine: "WhisperKit", model: "large-v3", locale: "en", runtime: "1.1.0", formatVersion: 1)
    ...
    for changed in [
      ...
      ImportTranscriptCacheIdentity(sourceChecksum: key.sourceChecksum, engine: key.engine, model: "small.en", locale: key.locale, runtime: key.runtime, formatVersion: 1),
  ...
  @Test func selectedWhisperIsNeverSilentlyReplaced() {
    let releases = ["small.en", "large-v3"].map {
      EngineReleaseRecord(id: UUID(), engineKey: WhisperModelCatalog.engineKey, version: $0, status: "installed", relativePath: nil)
    }
    #expect(WhisperModelSelection.active(from: releases, selected: "large") == .large)
    #expect(WhisperModelSelection.active(from: releases, selected: "tiny") == nil)
    #expect(WhisperModelSelection.active(from: releases, selected: "bad") == nil)
    #expect(TranscriptionModelStates.make(releases: releases, active: nil).first { $0.isActive }?.variant == .small)
  }
}
