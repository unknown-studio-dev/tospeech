...
struct AudioFirstPreparationTests {
  ...
  @Test func activePrefersDefaultAmongInstalled() {
    #expect(WhisperModelSelection.active(from: []) == nil)
    #expect(WhisperModelSelection.active(from: [release("small.en", "downloading")]) == nil)
    #expect(WhisperModelSelection.active(from: [release("large-v3", "installed")]) == .large)
    #expect(
      WhisperModelSelection.active(
        from: [release("large-v3", "installed"), release("small.en", "installed")]) == .small)
  }
  ...
  @Test func pureASRProducesNaturalSentences() async throws {
    ...
    let segments = try await AudioFirstPreparation.prepareSegments(
      ...
      variant: .small, sampleRate: 16_000, frameCount: 16_000 * 10,
      options: SentenceSegmentationOptions(pauseThreshold: 5, minDuration: 0),
      transcribe: { _, _ in asr })
  ...
  @Test func emptyTranscriptThrows() async {
    await #expect(throws: CaptionTranscriptError.self) {
      _ = try await AudioFirstPreparation.prepareSegments(
        audioURL: URL(fileURLWithPath: "/dev/null"), captionText: nil,
        variant: .small, sampleRate: 16_000, frameCount: 16_000,
        transcribe: { _, _ in [] })
    }
  }
}
